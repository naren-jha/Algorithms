...
public class LongestCommonSubstring {
	...
	class SimpleRecursiveSolution {
		...
		private int lcs(String s1, String s2, int m, int n, int lcs) {
			if (m == 0 || n == 0)
	            return lcs; 
	  
	        if (s1.charAt(m-1) == s2.charAt(n-1))
	            return lcs(s1, s2, m-1, n-1, lcs+1);
	        return Math.max(lcs, Math.max(lcs(s1, s2, m, n-1, 0), 
        			lcs(s1, s2, m-1, n, 0)));
		}
	}
	...
	class DPSolution {
		 ...
		 * if string1[m-1] and string2[n-1] doesn't equal to each other then 
	...
	public static void main(String[] args) {
        ...
        System.out.println(o.new SimpleRecursiveSolution().lcs(s1, s2)); // 4
        System.out.println(o.new DPSolution().lcsMem(s1, s2)); // 4
        System.out.println(o.new DPSolution().lcs(s1, s2)); // 4
	}
}
