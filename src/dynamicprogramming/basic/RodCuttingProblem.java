...
public class RodCuttingProblem {
    ...
    class DPSolution {
        ...
        public int cutRodMemoized(int[] price, int n) {
            ...
            return cutRodMemoizedUtil(price, n, mem);
        }
        
        private int cutRodMemoizedUtil(int[] price, int n, int[] mem) {
            if (mem[n] != -1)
                return mem[n];
            ...
            for (int i = 1; i <= n; i++)
                maxRevenue = Math.max(maxRevenue, 
                                     (price[i-1] + cutRodMemoizedUtil(price, n-i, mem)) );
            mem[n] = maxRevenue;
            return mem[n];
        }
        ...
        public int cutRodBottomUp(int[] price, int n) {
            ...
            for (int j = 1; j <= n; j++) {
                int maxRevenue = Integer.MIN_VALUE;
                for (int i = 1; i <= j; i++)
                    maxRevenue = Math.max(maxRevenue, (price[i-1] + dp[j-i]) );
                dp[j] = maxRevenue;
            }
            return dp[n];
        }
        ...
        private void printCutRodSolution(int[] price, int n) {
            ...
            for (int j = 1; j <= n; j++) {
                int maxRevenue = Integer.MIN_VALUE;
                for (int i = 1; i <= j; i++) {
                    if (maxRevenue < (price[i-1] + dp[j-i])) {
                        maxRevenue = (price[i-1] + dp[j-i]);
                        cuts[j] = i;
                    }
                }
                dp[j] = maxRevenue;
