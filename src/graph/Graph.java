...
public class Graph {
	...
	private void validateSourceAndDestinationVertices(int src, int dest) {
		if (src < 0 || src > numberOfVertices)
			throw new IllegalStateException("ivalid source vertex");
		if (dest < 0 || dest > numberOfVertices)
			throw new IllegalStateException("ivalid destination vertex");
	}
